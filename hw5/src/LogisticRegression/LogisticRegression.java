/**
	LogisticRegression: implementation of regularized logistic regression
**/
...
//	public static final double epsilon = 0.001;			// spambase, brease cancer
	...
	train(int fold) {
		...
		for (int iter = 0; iter < MAX_ITER; iter++) {
			...
			if (Math.abs(newObj - oldObj) < epsilon && iter != 0) {
//				break;
			}
			oldObj = newObj;
		}
	...
	test(int fold) {
		...
		System.out.println("Correct = " + cor + " Total = " + tot + " Accuracy = " + acc);
	...
	main(String[] args) {
		if (args.length != 2) {
			System.out.println("Usage: java LogisticRegression ../../data/spambase.csv <lambda (regularization coefficient)>");
			System.exit(0);
		}
...
//			if (fold != 0) break;
		...
		averAcc /= MAX_FOLD;
		System.out.println("Average Accuracy = " + averAcc);
	}
}
